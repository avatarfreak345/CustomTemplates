...
# Run Torch.Server.exe
TORCH_PATH="$SCRIPTDIR/Torch.Server.exe"
if [[ -f "$TORCH_PATH" ]]; then
  echo "Starting Torch.Server.exe..."
  /usr/bin/wine "$TORCH_PATH" >> torch_log.txt 2>&1
else
  echo "Torch.Server.exe not found at $TORCH_PATH" | tee -a torch_log.txt
fi
